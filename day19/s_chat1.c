...
int main(int argc, char* argv[]){
   ARGS_CHECK(argc, 3);
   int fdr,fdw;
   fdr=open(argv[1],O_RDONLY);//chat1管道1的读端
   ERROR_CHECK(fdr,-1,"open");
   fdw=open(argv[2],O_WRONLY);//chat1管道2的写端
   ERROR_CHECK(fdw,-1,"open");
    ...
    while(1){
        ...
        if(FD_ISSET(STDIN_FILENO,&rdset)){//此时读集合中标准输入是否有数据
            ...
            write(fdw,buf,sizeof(buf));//写入2号管道
        }
        if(FD_ISSET(fdr,&rdset)){//1号管道有数据
            memset(buf,0,sizeof(buf));
