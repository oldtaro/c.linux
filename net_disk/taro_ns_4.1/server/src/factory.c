        ...
        if(pcur){//发送发文件时也应判定，取到任务
            while(recv_cmd(&cmd,cfd),cmd.buf[0]){
                if(PUTS==cmd.cmd_type){
                    ...
                    bzero(md5,sizeof(md5));
                    ret=two_cond_seek(pcs->code,cmd.buf,md5,6);
...
int usr_puts_file(int cfd,char* md5,off_t* pfile_size){
    ...
    char file_name[100]={0},flag,compute_md5[50]={0};
    ...
    //文件未上传完成只更新文件池信息，传输完成才更新到用户文件信息
        ...
        *pfile_size=atoi(size);
        ...
        if(file_size==*pfile_size){
            ...
            *pfile_size=file_stat.st_size;
            ...
            if(*pfile_size<file_size) return 0;
            else{//传输完毕md5验证,验证通过才可更新，不然删除
                Compute_file_md5(file_name,compute_md5);
                if(!strcmp(md5,compute_md5)){
                    return 1;
                }else{
                    file_pool_delete(md5);
                    return 0;
                }
            }
        }
