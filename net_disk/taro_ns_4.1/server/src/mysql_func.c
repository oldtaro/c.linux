...
//文件删除文件池记录修改，引用计数减一，为0删除记录和文件
int file_pool_delete(char* md5)
{
	MYSQL *conn;
	MYSQL_RES *res;
	MYSQL_ROW row;
	...
	int index_count;
	char count[32]={0};
	char query[200]="select * from file_pool where md5";
	...
	if(index_count){
		...
		char query1[200]="delete from file_pool where md5";
		sprintf(query1,"%s='%s'",query1,md5);
		puts(query1);
		t=mysql_query(conn,query1);
		if(t)
		{
		...
		char sys_cmd[100]={0};
		sprintf(sys_cmd, "rm -f ../file_pool/%s",md5);
	    system(sys_cmd);
	}
	mysql_close(conn);
	return 0;
...
//文件信息表文件删除
int file_info_delete(char* file_name,int precode,int usr_id)
{
	MYSQL *conn;
	MYSQL_RES *res;
	MYSQL_ROW row;
	...
	int index_count;
	char count[32]={0};
	char query[200]="delete from file_info where file_name";
	sprintf(query,"%s='%s' and precode=%d and usr_id=%d",query,file_name,precode,usr_id);
	puts(query);
	unsigned int t;
	conn=mysql_init(NULL);//要初始化连接
	if(!mysql_real_connect(conn,server,user,password,database,0,NULL,0))
...
//双条件查询文件
int two_cond_seek(int precode,char* file_name,char* res_info,int res_num)
{
	MYSQL *conn;//连接
	MYSQL_RES *res;//结果操作指针
	MYSQL_ROW row;//一行的数据是存在row中间
	...
	//	printf("Query made...\n");
		...
		if(res)
		{
			if(row=mysql_fetch_row(res)){
				sprintf(res_info,"%s",row[res_num]);
				mysql_free_result(res);
				mysql_close(conn);
				return 1;
			}
