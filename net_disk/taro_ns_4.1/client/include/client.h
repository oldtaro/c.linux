...
int recv_total(int serfd,void* buf,off_t length,off_t offset);//循环接收文件
int recv_total_show(int serfd,void* buf,off_t length,off_t offset);//接收显示百分比
int send_total(int cfd,void* buf,off_t length,off_t offset);
int send_total_show(int cfd,void* buf,off_t length,off_t offset);
//接命令
...
//下载文件
int client_gets(char* file_name,int serfd);
//配置文件中读取ip,port
