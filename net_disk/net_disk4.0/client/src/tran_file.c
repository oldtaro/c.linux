...
int client_gets(char* file_name,int serfd){
    int ret,data_len;
    char path[100]={0},flag;
    recv(serfd,&flag,1,0);
    if(flag){
        sprintf(path,"./file/%s",file_name);
        //接收文件大小值
        ret=recv_total(serfd,&data_len,4,0);//接收文件大小值长度
        SELFFUNC_ERR_CHECK(ret,"recv_total");
        off_t file_size;
        ret=recv_total(serfd,&file_size,data_len,0);//接收文件大小值
        SELFFUNC_ERR_CHECK(ret,"recv_total");
        int fd=open(path,O_RDWR|O_CREAT,0666);
        ret=ftruncate(fd,file_size);
        ERROR_CHECK(ret,-1,"ftruncate");
        void* p=mmap(NULL,file_size,PROT_READ|PROT_WRITE,MAP_SHARED,fd,0);
        ret=recv_total_show(serfd,p,file_size,0);
        SELFFUNC_ERR_CHECK(ret,"recv_total");
