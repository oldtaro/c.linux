...
void* thread_mission(void* pf){
    ...
    client_state* pcs;
    cmd_train cmd;
    char file_name[50],md5[50];
    int cfd,offset,puts_fd;
    off_t file_size;
    while(1){
        ...
        if(pcur){//发送发文件时也应判定，取到任务
            while(recv_cmd(&cmd,cfd),cmd.buf[0]){
                if(PUTS==cmd.cmd_type){
                    ...
                    usr_puts_file(cfd,md5,&file_size);
...
int usr_puts_file(int cfd,char* md5,off_t* pfile_size){
    ...
    SELFFUNC_ERR_CHECK(ret,"recv_total");
    ret=file_is_exist(md5);

    if(ret){//文件已经存在文件池中，秒传;未传输完成文件，断点续传
        char file_size[64]={0};
        file_size_inpool(md5,file_size);
        *pfile_size=atoi(file_size);

        if(data_len==*pfile_size){
            flag=0;
            send(cfd,&flag,1,0);
            index_update(md5);//更新引用计数
            return 0;
